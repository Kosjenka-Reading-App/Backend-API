...
def read_user(user_id: int, db: Session = Depends(get_db), auth_user: schemas.AuthSchema = Depends(JWTBearer())):
...
def update_user(user_id: int, user: schemas.UserPatch, db: Session = Depends(get_db), auth_user: schemas.AuthSchema = Depends(JWTBearer())):
    db_user = crud.get_user(db, account_id = auth_user.account_id, user_id=user_id)
    if db_user is None:
        raise HTTPException(status_code=404, detail="User not found")
    db_user = crud.update_user(db, account_id = auth_user.account_id, user_id=user_id, user=user)
    return db_user


@app.delete("/users/{user_id}")
def delete_user(user_id: int, db: Session = Depends(get_db), auth_user: schemas.AuthSchema = Depends(JWTBearer())):
    db_user = crud.get_user(db,  account_id = auth_user.account_id, user_id=user_id)
    if db_user is None:
        raise HTTPException(status_code=404, detail="User not found")
...
def create_user(user: schemas.UserCreate, db: Session = Depends(get_db), auth_user: schemas.AuthSchema = Depends(JWTBearer())):
