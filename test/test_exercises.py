...
def test_update_exercise_modifies_date(admin_token):
    ...
    res_exercise = client.post("http://localhost/exercises/", json=exercise_data, headers=auth_header(admin_token))
    ...
    response = client.patch(f"http://localhost/exercises/{exercise_id}", json=exercise_data, headers=auth_header(admin_token))
