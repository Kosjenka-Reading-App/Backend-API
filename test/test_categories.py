...
def test_create_category(category_name, admin_token):
    categories = client.get("http://localhost:8000/categories", headers=auth_header(admin_token)).json()
    category_count = len(categories)
    created_category = client.post(
        f"http://localhost:8000/categories/{category_name}", headers=auth_header(admin_token)
    ).json()
...
def test_get_categories(regular_token):
    categories = client.get("http://localhost:8000/categories", headers=auth_header(regular_token)).json()
...
def test_update_category(admin_token):
    ...
    updated_category = client.patch(
        f"http://localhost:8000/categories/{original_category}", json=body, headers=auth_header(admin_token)
    ).json()
    assert updated_category["category"] == "Mice"
    assert (
        original_category not in client.get("http://localhost:8000/categories", headers=auth_header(admin_token)).json()
    )
...
def test_delete_category(admin_token):
    ...
    while categories:
        ...
        client.delete(f"http://localhost:8000/categories/{category}", headers=auth_header(admin_token)).json()
        remaining_categories = client.get("http://localhost:8000/categories", headers=auth_header(admin_token)).json()
...
def test_sort_categories(regular_token):
    ...
    print(categories)
    assert len(categories) > 0
    assert client.get("http://localhost:8000/categories?order=asc", headers=auth_header(regular_token)).json() == sorted(
        categories
    )
    assert (
        client.get("http://localhost:8000/categories?order=desc", headers=auth_header(regular_token)).json()
        == sorted(categories)[::-1]
    )


def test_search_categories(regular_token):
    categories = client.get("http://localhost:8000/categories?name_like=use", headers=auth_header(regular_token)).json()
    assert len(categories) > 0
    print(categories)
